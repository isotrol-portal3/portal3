 ...
 */
@Component
public final class AggregationComponent extends AbstractMapComponent<String> {
	/** Default constructor. */
	...
	Long compute(String key) {
		AggregationEntity entity = getDao().getAggregation(key);
		if (entity == null) {
			entity = new AggregationEntity();
			entity.setName(key);
			getDao().save(entity);
			flush();
		}
