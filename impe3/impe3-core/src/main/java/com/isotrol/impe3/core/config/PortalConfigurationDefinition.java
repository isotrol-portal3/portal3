...
/**
 * Portal configuration definition.
 * @author Andres Rodriguez.
 * 
 * @param <T> Portal configuration interface.
 */
public final class PortalConfigurationDefinition<T extends PortalConfiguration> extends Definition<T> {
	/** Cache. */
	private static final SingleValueSupport<Class<?>, PortalConfigurationDefinition<?>> CACHE = SingleValueSupport.create();

	/** Predicate to evaluate if a class is a configuration. */
	public static final Predicate<Class<?>> IS_PORTAL_CONFIGURATION = Classes.extendsOrImplements(PortalConfiguration.class);
	 ...
	 */
	public static <T extends PortalConfiguration> PortalConfigurationDefinition<T> of(Class<T> configClass)
		throws ConfigurationException {
		Preconditions.checkNotNull(configClass, "A configuration class must be provided");
		@SuppressWarnings("unchecked")
		final PortalConfigurationDefinition<T> d1 = (PortalConfigurationDefinition<T>) CACHE.get(configClass);
		if (d1 != null) {
			return d1;
		}
		@SuppressWarnings("unchecked")
		final PortalConfigurationDefinition<T> d2 = (PortalConfigurationDefinition<T>) CACHE.put(configClass,
			new PortalConfigurationDefinition<T>(configClass));
		return d2;
	}
	 ...
	 */
	public static PortalConfigurationDefinition<?> of(String name) throws ConfigurationException {
		Preconditions.checkNotNull(name, "A configuration class name must be provided");
		try {
			final Class<?> klass = Class.forName(name);
			final Class<? extends PortalConfiguration> configClass = klass.asSubclass(PortalConfiguration.class);
	...
	private static ImmutableMap<String, Item> getItems(Class<? extends PortalConfiguration> configClass)
		throws ConfigurationException {
	 ...
	 * @throws ConfigurationException if the configuration class is invalid.
	 */
	private PortalConfigurationDefinition(Class<T> configClass) throws ConfigurationException {
		super(configClass);
	 ...
	 * @return
	 */
	public PortalConfigurationBuilder<T> builder() {
		return new Builder();
	}
	...
	public static final class Item implements Named {
		...
		private Item(Class<? extends PortalConfiguration> configClass, final Method m) {
			...
			this.type = ConfigurationType.ofPortal(configClass, parameter, klass);
	 ...
	 * @author Andres Rodriguez.
	 */
	private final class Builder implements PortalConfigurationBuilder<T> {
		/** Values. */
