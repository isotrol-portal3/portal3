...
/**
 * Device Resolver Chain Module
 * @author Emilio Escobar Reyero
 */
@Path("devicechain.xml")
@Bundle
@Name("name")
@Description("desc")
@Copyright("Isotrol, SA. (GPL3)")
@Version("1.0")
public interface DeviceResolverChainModule extends Module{
	/** Provided service. */
	@Name("resolver")
	DeviceResolver service();

	@Name("resolver.first")
	void firstStep(DeviceResolver resolver);
	
	@Name("resolver.second")
	void secondStep(DeviceResolver resolver);

	@Name("resolver.third")
	void thirdStep(DeviceResolver resolver);

	@Name("resolver.fourth")
	void fourthStep(DeviceResolver resolver);

	@Name("resolver.fifth")
	void fifthStep(DeviceResolver resolver);
}
