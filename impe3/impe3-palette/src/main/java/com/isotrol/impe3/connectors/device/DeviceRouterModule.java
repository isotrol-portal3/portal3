 ...
 */
@Path("device-router.xml")
@Bundle
@Name("name")
@Description("desc")
@Copyright("Isotrol, SA. (GPL3)")
@Version("1.0")
public interface DeviceRouterModule extends Module {
	@Name("name")
	DeviceRouter service();
	
	@Name("device.resolver")
	void deviceResolver(DeviceResolver deviceResolver);
	
	@Name("device.generator")
	void deviceURIGenerator(DeviceURIGenerator deviceURIGenerator);
	
	void config(DeviceRouterModuleConfig config);
