...
/**
 * Component to apply a filter by category.
 * @author Andres Rodriguez
 * 
 */
@Name("Filtro por Clave de Navegaci\u00f3n")
public class CategoryFilterComponent implements Component {
	/** Configuration. */
	private CategoryFilterConfig config;
	...
	@Inject
	public void setConfig(CategoryFilterConfig config) {
		this.config = config;
	}
	...
	public ComponentResponse execute() {
		criteriaTransformer = new ContentCriteriaTransformer() {
			...
			public void apply(ContentCriteria criteria) {
				criteria.categories().apply(config.category(), config.filterType().getFilterType());
			}
		};
