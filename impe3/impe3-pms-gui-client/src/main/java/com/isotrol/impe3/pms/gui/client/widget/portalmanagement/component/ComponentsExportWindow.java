...
/**
 * The window with own portal components that can be exported.<br/> .
 * 
 * @author Manuel Ruiz
 * 
 */
public class ComponentsExportWindow extends TypicalWindow {
	/**
	 ...
	 * Components grid.<br/>
	 */
	private Grid<ModuleInstanceSelModelData> gComponents = null;
	...
	private void tryGetComponents() {
		...
		AsyncCallback<List<ModuleInstanceSelDTO>> callback = new AsyncCallback<List<ModuleInstanceSelDTO>>() {
			...
			public void onSuccess(List<ModuleInstanceSelDTO> arg0) {
				...
				List<ModuleInstanceSelModelData> lComponentsModelData = new LinkedList<ModuleInstanceSelModelData>();
				for (ModuleInstanceSelDTO dto : arg0) {
					lComponentsModelData.add(new ModuleInstanceSelModelData(dto));
				}
				sGrid.removeAll();
		...
		componentsService.getComponents(portalId, callback);
	}
	...
	private void addGrid() {
		...
		gComponents = new Grid<ModuleInstanceSelModelData>(sGrid, cm);

		gComponents.setSelectionModel(selectionModel);
		gComponents.addPlugin(selectionModel);

		gComponents.setAutoExpandColumn(ModuleInstanceSelModelData.PROPERTY_DESCRIPTION);
	...
	private void addToolbar() {
		...
		SelectionListener<ButtonEvent> ttiExportSelectedListener = new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
				GridSelectionModel<ModuleInstanceSelModelData> selectionModel = gComponents.getSelectionModel();
		...
		StoreFilterField<ModuleInstanceSelModelData> filter = new CustomizableStoreFilter<ModuleInstanceSelModelData>(Arrays
			.asList(new String[] {ModuleInstanceSelModelData.PROPERTY_NAME, ModuleInstanceSelModelData.PROPERTY_DESCRIPTION}));
	...
	private void tryExporSelectedComponents(Set<String> ids) {
		...
		componentsService.exportSome(portalId, ids, callback);
	}
	...
	private void tryExportAllComponents() {
		...
		componentsService.exportAll(portalId, callback);
	}
