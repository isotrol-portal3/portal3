 ...
 * 
 */
public class SessionErrorMessageResolver extends APmsErrorMessageResolver {
	...
	protected String getMessage(Throwable error) {
		if (error instanceof LockedUserException) {
			return getPmsMessages().exceptionLockedUser() + "<br/>" + getPmsMessages().contactAdminMessage();
		} else if (error instanceof InactiveUserException) {
			return getPmsMessages().exceptionInactiveUser() + "<br/>" + getPmsMessages().contactAdminMessage();
		}
	 ...
	 * @return
	 */
	public String getErrorLoginMessage(Throwable error, String altMessage) {
		if (error instanceof LockedUserException) {
			return getPmsMessages().exceptionLockedUser() + "<br/>" + getPmsMessages().contactAdminMessage();
		} else if (error instanceof InactiveUserException) {
			return getPmsMessages().exceptionInactiveUser() + "<br/>" + getPmsMessages().contactAdminMessage();
		...
		return altMessage != null ? altMessage : super.getMessage(error);
	}
}
