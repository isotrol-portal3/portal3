...
/**
 * Error message resolver for exceptions thrown by Pages service method calls
 * 
 * @author Andrei Cojocaru
 *
 */
public class PageErrorMessageResolver extends APmsErrorMessageResolver {
	/* (non-Javadoc)
	...
	protected String getMessage(Throwable error) {
		if (error instanceof PageInUseException) {
			return getPmsMessages().exceptionPageInUse();
		} else if (error instanceof PageNotFoundException) {
			return getPmsMessages().exceptionPageNotFound();
		}
